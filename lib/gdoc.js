...
async function getDoc(publicURL, locale = '_tw') {
  ...
  for(const key in stickers) {
    ...
    gdoc = gdoc.replace(regex, `<div class="sticker ${classes}"><div>${label}</div></div>`) // <img class="sticker inline" scr="/images/${label}.png" />
  }
  // gdoc = gdoc.replace(/{{\s?wp\s?}}/g, '<img src="/images/wp.png" class="sticker inline" />')

  // gdoc = gdoc.replace(/{{\s?weOK\s?}}/g, '<img src="/images/we-ok.png" class="sticker inline" />')

  // gdoc = gdoc.replace(/{{\s?iorgOpinion\s?}}/g, '<div class="label inline">IORG 觀點</div>')
