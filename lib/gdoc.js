...
async function getDoc(publicURL) {
  ...
  if(metaTableEl.length > 0) {
    ...
    if(fieldElements.length >= 5) {
      ...
      if(fieldElements.length > 5) {
        moreInfo = fieldElements.eq(5).text()
      }
  ...
  for(let i = 0; i < tables.length; i++) {
    ...
    if(type === 'photo') {
      ...
      const classes = ['gdoc-post-container', ...containerClasses].join(' ')
      ...
      const headerHTML = type === 'def' ? cells.eq(1).html() : (s > 1 ? cells.eq(1).html() : null)
