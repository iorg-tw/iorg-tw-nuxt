  ...
  for(let i = 0; i < tables.length; i++) {
    ...
    if(type === 'photo') {
      ...
      headerRow.find('td').each((i, el) => {
        ...
        const ctrlRegex = /{{[0-9a-zA-Z-^}]+}}/g
        const matches = thText.match(ctrlRegex)
        if(matches) {
          colClassLists.push(matches.map(extractMatchContent))
          th.html('<p>' + thText.replace(ctrlRegex, '').trim() + '</p>') // gdoc cell text are all wrapped in p
        } else {
          colClassLists.push([])
        }
        th.get(0).tagName = 'th'
      ...
      trs.each((trIndex, trEl) => {
        ...
        tds.each((tdIndex, tdEl) => {
          ...
          if(classList.includes('ul-set')) {
            tr.find('ul > li').each((liIndex, liEl) => {
              const li = $(liEl)
              const html = li.html()
