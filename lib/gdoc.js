...
async function getDoc(publicURL, locale = '_tw') {
  ...
  gdoc = gdoc.replace(/(X)(-[0-9A-Z]{4}-[0-9A-Z]{4})/g, '<a href="/archive/$1$2" target="_blank" class="ioid"><span>X</span><span>$2</span></a>')
    ...
    if(type === 'end') {
      ...
      trs.each((trIndex, trEl) => {
        ...
        tds.each((tdIndex, tdEl) => {
          const td = $(tdEl)
          const colClassListAtIndex = colClassLists[tdIndex] ? colClassLists[tdIndex] : [] // FIXME: temp fix
  ...
  for(const key in stickers) {
    ...
    gdoc = gdoc.replace(regex, `<div class="sticker ${classes}"><div>` + (label ? label : '') + '</div></div>')
  }
