...
async function getDocImage(src) {
  const response = await axios.get(src, { responseType: 'arraybuffer' })
  const contentType = response.headers['content-type'] || 'image/jpeg'
  const extension = contentType.includes('image/png') ? 'png' : 'jpg'
  const urlObj = new URL(src)
  let fileName = urlObj.pathname.split('/')
  fileName = fileName[fileName.length - 1] + '.' + extension
...
async function getDoc(publicURL, locale = '_tw', parseSections = false) {
  ...
  gdoc = gdoc.replace(/(DA-)([1-9][0-9]?[0-9]?)/g, '<a href="/da/$2" target="_blank" class="da"><span>DA $2</span></a>') // make DA link
  ...
  let title = '標題未定'
  if(titleEl.length > 0) {
    title = (titleEl.length > 1 ? titleEl.eq(1) : titleEl.eq(0)).text()
    titleEl.remove()
  }
  ...
  if(metaTableEl.length > 0) {
    ...
    if(fieldElements.length >= 5) {
      ...
      if(coverImageMatch) {
        ...
        docImages[coverImage] = await getDocImage(coverImage)
      } else {
  ...
  const conditionalFormatting = {
    danger: ['刪除'],
    nil: ['否', '無', '未發現', '未提及', 'No', 'None', '相同', '暫無', '暫無資料', '無法辨識', '無法計算', '無法證偽', 'Unable to disproof', '同上'],
    yes: ['✓', '是', '有', '有證據', '有提及', 'Yes', 'Proven']
  }
  const conditionalFormattingRank = {
    pos: /^[+][0-9]+?$/,
    neg: /^[-][0-9]+?$/,
    nil: /-/
  }
  ...
  for(let i = 0; i < tables.length; i++) {
    ...
    if(type === 'end') {
      ...
      for(let x = 0; x < images.length; x++) {
        const el = images.eq(x)
        const src = el.attr('src')
        docImages[src] = await getDocImage(src)
      }
      ...
      table.after(el)
      table.remove()
    } else if(['quote', 'def', 'note', 'preview'].includes(type)) {
      const s = cells.length - 1
      ...
      // make container
      const classesOut = ['gdoc-dict', 'gdoc-card-container', ...firstCellClasses].join(' ')
      const classesIn = ['cards', 'container'].join(' ')
      const container = $(`<div class="${classesOut}"><div class="${classesIn}"></div></div>`)
      ...
      // make cards
      const cardsContainer = container.find('.cards')
      const cards = getCards($, table, 'term')
      for(const card of cards) {
        const panel = $('<div class="panel tiled compact"></div>')
        const term = $('<h3 class="term">' + card.term + '</h3>')
        panel.append(term)
        if(card.def) {
          panel.append('<p class="def">' + card.def + '</p>')
        }
        if(card.desc) {
          panel.append('<p class="desc">' + card.desc + '</p>')
        }
        cardsContainer.append(panel)
      }
      table.after(container)
      table.remove()
    } else if(type === 'cards') {
      const classes = ['gdoc-cards', 'gdoc-card-container', ...firstCellClasses].join(' ')
      ...
      const classes = ['gdoc-numbers', 'gdoc-card-container', ...firstCellClasses].join(' ')
      const container = $(`<div class="${classes}"></div>`)
      ...
      const classes = ['gdoc-quotes', 'gdoc-card-container', ...firstCellClasses].join(' ')
      const container = $(`<div class="${classes}"></div>`)
      ...
      const classes = [`gdoc-${type}`, 'gdoc-card-container', ...firstCellClasses].join(' ')
      const container = $(`<div class="${classes}"></div>`)
      ...
      const classes = ['gdoc-events', 'gdoc-card-container', ...firstCellClasses].join(' ')
      const container = $(`<div class="${classes}"></div>`)
      ...
      const classes = ['gdoc-awards', 'gdoc-card-container', ...firstCellClasses].join(' ')
      const container = $(`<div class="${classes}"></div>`)
      ...
      trs.each((trIndex, trEl) => {
        ...
        tds.each((tdIndex, tdEl) => {
          ...
          if(tdChildren.length === 1 && tdParagraphs.length === 1 && tdImages.length === 0 && tdAnchors.length === 0) {
            ...
            } else if(tdText.match(/^[0-9]{2}-[0-9]{2}$/)) {
              tdClasses.push('month-date')
            }
            if(tdText.match(/^[+-]?([0-9]+)?[+]?$/) && tdClasses.includes('col-rank')) { // this column is a ranking
              tdClasses.push('rank')
              for(const [className, regex] of Object.entries(conditionalFormattingRank)) {
                if(tdText.match(regex)) {
                  tdClasses.push(className)
                  break
                }
              }
