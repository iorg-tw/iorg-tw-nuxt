...
async function getDoc(publicURL) {
  ...
  gdoc = gdoc.replace(/(X-[0-9A-Z]{2}-[0-9]{2}-[0-9]{2})/g, '<a href="https://iorg.tw/archive/$1" class="archive">$1</a>')
  ...
  if(metaTableEl.length > 0) {
    ...
    if(fieldElements.length >= 5) {
      ...
      tags = fieldElements.eq(4).text().trim()
      ...
      tags = tags.length > 0 ? tags.split(PUNCT.PAUSE).filter(tag => tag !== '') : []
      metaTableEl.remove()
    }
  }
