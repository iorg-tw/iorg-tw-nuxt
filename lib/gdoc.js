...
const parseControls = (content) => {
  ...
  const attributes = {}
  if(matches) {
    content = content.replace(ctrlRegex, '')
...
async function getDoc(publicURL) {
  ...
  gdoc = gdoc.replace(/(X-[0-9A-Z]{4}-[0-9A-Z]{4})/g, '<a href="/archive/$1" class="ioid">$1</a>')

  gdoc = gdoc.replace(/{{\s?wp\s?}}/g, '<img src="/images/wp.png" class="wp" />')

  gdoc = gdoc.replace(/{{\s?weOK\s?}}/g, '<img src="/images/we-ok.png" class="we-ok" />')
