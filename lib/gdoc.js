...
async function getDoc(publicURL, locale = '_tw') {
  ...
  gdoc = gdoc.replace(/(X)(-[0-9A-Z]{4}-[0-9A-Z]{4})/g, '<a href="/archive/$1$2" target="_blank" class="ioid"><span>X</span><span>$2</span></a>') // make archive link
  ...
  for(let i = 0; i < tables.length; i++) {
    ...
    if(type === 'end') {
      ...
      const el = $(`<div class="${classes}">` + `<div class="detail">${contentHTML}</div>` + (descriptionHTML ? `<div class="description">${descriptionHTML}</div>` : '') + '</div>')
      table.after(el)
      table.remove()
      ...
      trs.each((trIndex, trEl) => {
        ...
        tds.each((tdIndex, tdEl) => {
          const td = $(tdEl)
          const colClassListAtIndex = tdIndex < colClassLists.length ? colClassLists[tdIndex] : [] // FIXME: temp
          const classList = [...colClassListAtIndex]
          const text = td.text()
          ...
          if(tdChildren.length === 1 && tdChildren.filter('p').length === 1) { // only 1 p in td
            const { content: tdContent, classes: tdClasses } = parseControls(text)
            classList.push(...tdClasses)
            td.html(`<p>${tdContent}</p>`)
          }
  ...
  // stickers
  // FIXME: this might be too violent
  ...
  for(const key in stickers) {
    ...
    gdoc = gdoc.replace(regex, `<label class="sticker ${classes}"><span class="content">` + (label ? label : '') + '</span></label>')
  }

  // query str
  gdoc = gdoc.replace(/{{\s?query\s?=\s?([^}]+)\s?}}/g, (match, query = '') => {
    query = query.trim()
    query = query.replace(/\[([^\]]+)\]/g, '$1') // FIXME: nested span is weird...
    return '<div class="query">' + query + '</div>'
  })
