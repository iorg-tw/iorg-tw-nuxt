...
function structureDoc(HTML, H) {
  ...
  console.info('structureDoc', totalLevels, 'levels')
  const _f = (html, hierarchy) => {
    ...
    console.info(indentation + '_f', t0, JSON.stringify(hierarchy))
    $(t0).each((i, el) => {
      console.info(indentation + 'find', t0)
      let header = $(el)
      let body = header.nextUntil(t0)
      console.info(indentation + 'found', t0, header.text(), body.text())
      header = $('<div></div>').append(header).html()
      ...
      if(hierarchy.length < 2) {
        ...
        const next$ = cheerio.load(body)
        const until = hierarchy[1]
        let bodyBeforeChildren = next$('body').children(until).first().prevAll().get().reverse()
        bodyBeforeChildren = $('<div></div>').append(bodyBeforeChildren).html()
        sibling = {
          header,
          body: bodyBeforeChildren,
          children: _f(body, hierarchy.slice(1))
        }
      }
