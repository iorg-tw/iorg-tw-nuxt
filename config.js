const config = {
  title: 'IORG – Information Operations Research Group',
  description: 'IORG is a Taiwan-based multidisciplinary research group focuses on information operations within the framework of cognitive warfare.',
  cover: '/images/cover-2-1.png',
  intro: 'A multidisciplinary, data-driven, rigorous understanding of information operations is needed in order to devise appropriate responses to respond to cognitive warfare, protect Taiwan’s civil liberties, and strengthen Taiwan’s democracy.',
  researcherSectionHeader: 'Researchers at IORG:',
  researchers: [
    {
      name: 'Josh Wang',
      description: 'Activist, political analyst.'
    },
    {
      name: 'Chihhao Yu',
      description: 'Technologist, designer, civic hacking community g0v contributor.'
    },
    {
      name: 'Hsiaochiang Hsu',
      description: 'European political economy, former legislative assistant.'
    },
    {
      name: 'Jin-Kun Lin',
      description: 'Backend engineer.'
    },
    {
      name: 'Wesley Chia-Wei Lin',
      description: 'Data researcher.'
    },
    {
      name: 'Wan-Ling Yen',
      description: 'Activist, community organizer and researcher.'
    },
    {
      name: 'Tian-You Chen',
      description: 'Political researcher.'
    }
  ],
  consultantSectionHeader: 'Consultants to IORG:',
  consultants: [
    {
      name: 'Puma Shen',
      description: 'Information warfare. Assistant professor of criminology at National Taipei University.'
    },
    {
      name: 'Ming-Hung Wang',
      description: 'Social media analysis, information security. Assistant professor of computer science at Feng-Chia University.'
    }
  ],
  reports: [
    {
      title: '中國對台資訊作戰事件研析，IO 研究室，2020/6/18。',
      image: '/images/iorg-model-1-tw-cover.png',
      actions: [
        {
          label: '下載報告',
          url: 'https://github.com/iorg-tw/documents/raw/master/iorg-model-1-tw.pdf'
        }
      ]
    },
    {
      title: 'Event Studies of Chinese Information Operations Against Taiwan, IORG, 2020/5/22, last updated 2020/6/9.',
      image: '/images/iorg-model-1-cover.png',
      actions: [
        {
          label: 'Download Report',
          url: 'https://github.com/iorg-tw/documents/raw/master/iorg-model-1.pdf'
        },
        {
          label: 'Download Appendix',
          url: 'https://github.com/iorg-tw/documents/raw/master/iorg-model-1-appendix.pdf'
        }
      ]
    }
  ]
}

module.exports = config
